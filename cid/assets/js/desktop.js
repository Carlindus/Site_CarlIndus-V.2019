...
/*
 * Add actions on all icons of the desktop defined in the configuration file
 */
function initDesktop(configContentPath) {
  var icons = getJsonText(configContentPath);

  addFocusArticlesOnclick();
...
//TODO améliorer le focus
function addFocusArticlesOnclick() {
  $("article").click(function (event) {
    console.log(event.target);
    if (event.target.classList.contains('ico')) {
      console.log("icons");
    } else {
      console.log("pas icons");
    }

    var target = $(event.currentTarget);
    getFocusOnArticle(target);
  })
}

function getFocusOnArticle(target) {
  target.css({ "z-index": ++currentZIndex });
}
/*
...
function addEventListenerOpenWindow(iconId) {
  $('#' + iconId + '-ico').click(function (event) {
    getFocusOnArticle($('#' + iconId));
    openWindow(iconId);
  });
}
...
/*
 * Add draggable behaviour for the directory
 */
